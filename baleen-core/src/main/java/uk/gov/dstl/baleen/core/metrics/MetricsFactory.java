 ...
 *
 * <p>For simplicity of use across Baleen this class acts as both a singleton and a baleen component
 * instance. You must call configure/start/stop to use this, which {@link BaleenManager} takes care
 * of.
 *
 * <p>Configuration through YAML as follows (all reporters have the generic configuration options of
 * console above)
 *
 * <pre>
 * metrics:
 ...
 *
 * <p>Allowable types are log (through standard logging), CSV (output to CSV files), console (send
 * to the console), elasticsearch (send to a remote server). Instances of the reporters are created
 * through {@link ReporterUtils}.
 */
public class MetricsFactory implements BaleenComponent {
  ...
  /** A reporter that knows how to start and stop itself with a pre-configured reporting period. */
  private static class ConfiguredReporter {
     ...
     * @param period period in milliseconds. Values less than zero mean that the reporter should not
     *     be scheduled, but may be used ad hoc through report function.
     */
