...
public class AbstractComponentApiServlet extends AbstractApiServlet {
   ...
   * Constructor
   *
   * @param componentClass The name of the supertype for which we are listing subclasses
   * @param componentPackage The default package name, which will be stripped from class names
   * @param excludeClass A list of classes of which subclasses will be excluded from the listing
   * @param excludePackage A list of regular expressions against which packages will be tested and,
   *     if matched, excluded from the listing
   * @param clazz The subclass of AbstractComponentApiServlet, for creating a logger
   */
  public AbstractComponentApiServlet(
  ...
  private void calculateComponents() {
    ...
    try {
      ...
      LoggerFactory.getLogger(clazz)
          .warn("Unable to find component class - annotator listing will not be available", e);
    }
    ...
    for (String c : excludeClass) {
      try {
        ...
        LoggerFactory.getLogger(clazz)
            .warn("Unable to find component class - component will not be excluded", e);
      }
    }
    ...
    if (componentClazz == null) {
      ...
      List<String> componentsList =
          classesToFilteredList(
              ReflectionUtils.getSubTypes(componentClazz),
              componentPackage,
              excludeClazz,
              excludePackage);
   ...
   * @param excludePkg A list of RegEx patterns that describe the set of packages to test against
   ...
   * @param excludeClazz A list of classes to test against
  ...
  protected <S extends T, T> Class<S> getClassFromString(String className, String... defaultPackage)
    ...
    for (String pkg : defaultPackage) {
      try {
        ...
        LoggerFactory.getLogger(clazz)
            .debug("Couldn't find class {} in package {}", className, pkg, e);
      }
    }
    ...
    try {
      ...
      throw new InvalidParameterException(
          "Could not find or instantiate analysis engine " + className, e);
    }
  }
