...
public class ReferentUtils {
  ...
  public static final Predicate<? super Base> NOT_ENTITY_OR_REFERENT =
      e -> !(e instanceof Entity) && e.getReferent() == null;

  public static final Predicate<? super Base> ENTITY_OR_REFERENT =
      e -> e instanceof Entity || e.getReferent() != null;
   ...
   * @param convert the conversion function which will convert a list to a single entry. This does
   *     not have to be a value from the list, and may be null (though that will drop the
   *     corresponding key).
   * @return the map
   */
  public static <T> Map<ReferenceTarget, T> filterToSingle(
   ...
   *
   * <p>Effectively get any annotation where the covered text is considered to be an entity.
   *
   * @param jCas the j cas
   * @param referentMap the referent map
   * @return the all entity or referent to entity
   */
  public static List<Base> getAllEntityOrReferentToEntity(
      JCas jCas, Map<ReferenceTarget, Entity> referentMap) {
  ...
  public static <T extends Base> T getLongestSingle(Collection<T> list) {
    return singleViaCompare(
        list, (a, b) -> Integer.compare(a.getCoveredText().length(), b.getCoveredText().length()));
  }
   ...
   * @return the set
   */
  public static Set<Entity> replaceWithCoreferent(
      Collection<Entity> entities, Map<ReferenceTarget, Entity> referentMap) {
