 ...
 * JCasGen Tue Apr 12 12:06:57 BST 2016
 *
 * @generated
 */
public class ProtectiveMarking_Type extends BaleenAnnotation_Type {
  ...
  public static final boolean featOkTst =
      JCasRegistry.getFeatOkTst("uk.gov.dstl.baleen.types.metadata.ProtectiveMarking");
  ...
  public String getCaveats(int addr, int i) {
    ...
    if (lowLevelTypeChecks)
      return ll_cas.ll_getStringArrayValue(
          ll_cas.ll_getRefValue(addr, casFeatCode_caveats), i, true);
  ...
  public String getReleasability(int addr, int i) {
    ...
    if (lowLevelTypeChecks)
      return ll_cas.ll_getStringArrayValue(
          ll_cas.ll_getRefValue(addr, casFeatCode_releasability), i, true);
  ...
  public void setReleasability(int addr, int i, String v) {
    ...
    if (lowLevelTypeChecks)
      ll_cas.ll_setStringArrayValue(
          ll_cas.ll_getRefValue(addr, casFeatCode_releasability), i, v, true);
    jcas.checkArrayBounds(ll_cas.ll_getRefValue(addr, casFeatCode_releasability), i);
  ...
  public ProtectiveMarking_Type(JCas jcas, Type casType) {
    ...
    casFeat_classification =
        jcas.getRequiredFeatureDE(casType, "classification", "uima.cas.String", featOkTst);
    casFeatCode_classification =
        (null == casFeat_classification)
            ? JCas.INVALID_FEATURE_CODE
            : ((FeatureImpl) casFeat_classification).getCode();

    casFeat_caveats =
        jcas.getRequiredFeatureDE(casType, "caveats", "uima.cas.StringArray", featOkTst);
    casFeatCode_caveats =
        (null == casFeat_caveats)
            ? JCas.INVALID_FEATURE_CODE
            : ((FeatureImpl) casFeat_caveats).getCode();

    casFeat_releasability =
        jcas.getRequiredFeatureDE(casType, "releasability", "uima.cas.StringArray", featOkTst);
    casFeatCode_releasability =
        (null == casFeat_releasability)
            ? JCas.INVALID_FEATURE_CODE
            : ((FeatureImpl) casFeat_releasability).getCode();
  }
}
