 ...
 *
 * <p>For example, "William Tell (Bill)" would coreference William Tell and Bill if they were both
 * person entities.
 *
 * @baleen.javadoc
 */
public class CorefBrackets extends BaleenAnnotator {
                ...
                .filter(
                    f ->
                        f.getClass().isAssignableFrom(e.getClass())
                            || e.getClass().isAssignableFrom(f.getClass()))
  ...
  private void makeCoref(JCas jCas, Collection<Entity> entities) {
    ...
    if (rts.isEmpty()) {
      ...
      if (mergeReferents) {
        ...
        getMonitor()
            .warn(
                "Multiple existing referents found, only those entities without existing referents will be modified");
  ...
  /**
   * Set the referents of all entities in <em>entities</em> to the ReferenceTarget specified by
   * <em>target</em>
   */
  ...
  /**
   * For all entities in <em>entities</em> that don't have an existing referent, set them to the
   * same new ReferenceTarget. This ReferenceTarget is returned if created, else null is returned.
   */
  ...
  /**
   * Sets the referents to <em>target</em> on all entities that either have a null referent, or a
   * referent that matches one of the targets specified in <em>condition</em>
   */
  private void setReferents(
      ReferenceTarget target, Collection<Entity> entities, Collection<ReferenceTarget> condition) {
    for (Entity e : entities) {
