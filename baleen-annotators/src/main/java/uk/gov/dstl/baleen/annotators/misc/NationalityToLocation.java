...
/**
 * Take Nationality entities and add corresponding Location entities so that nationalities can be
 * visualised
 *
 * <p>For each nationality entity, use the country code to find the GeoJSON of the country it is
 * linked to and add a new Location entity to the CAS so that we can visualise nationalities.
 */
public class NationalityToLocation extends BaleenAnnotator {
    ...
    for (Nationality nationality : nationalities) {
      if (!Strings.isNullOrEmpty(nationality.getCountryCode())) {
        ...
        if (!Strings.isNullOrEmpty(geoJson)) {
          ...
          getMonitor()
              .debug(
                  "Unable to find location (with GeoJSON) for country code '{}'",
  ...
  public AnalysisEngineAction getAction() {
    return new AnalysisEngineAction(
        ImmutableSet.of(Nationality.class), ImmutableSet.of(Location.class));
  }
}
