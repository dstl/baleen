...
/**
 * Annotate Latitude-Longitude coordinates in decimal (DD) and degrees-minutes-seconds (DMS) format
 * using regular expressions.
 *
 * <p><b>Decimal degree (DD)</b>
 *
 * <p>The document content is run through a regular expression matcher looking for
 * latitude-longitude pairs in DD (e.g. 51.068787 -1.794472 or 51.068787° -1.794472° or 51.068787°N
 * 1.794472°W). If the minimum number of decimal places required is 0, the following regular
 ...
 *
 * <p>If the minimum number of decimal places required is greater than 0, the following regular
 * expressions are used where x is the minimum number of decimal places:
 *
 ...
 *
 * <p>The latitude and longitude are extracted (optionally the user can specify it should be
 * longitude and latitude instead) and a GeoJSON object is built representing this location.
 *
 * <p>Coordinates that are preceded by a £, $ or € symbol are skipped as they are assumed to be
 * monetary values rather than coordinates (e.g. £3,000).
 *
 * <p>The regular expressions used when searching for Digital Degree formats that use the minus sign
 * for southern and western hemispheres do not include word boundaries when looking for matches
 * because this can cause the expression to fail to extract a first negative sign. This means the
 * annotator will extract, for example, currency strings (e.g. $40,000).
 *
 * <p><b>Degrees-minutes-seconds (DMS)</b>
 *
 * <p>The document content is run through a regular expression matcher looking for
 * latitude-longitude pairs in DMS format (e.g. 51°4'7.6332"N 1°47'40.0992"W) that match the
 ...
 * </pre>
 *
 * <p>A similar regex is used to find pairs that have spaces instead of symbols in them (e.g. 10 12
 * 14 N, 11 13 15 E), and another regex to find pairs with no spaces.
 *
 * <p>The following regexes are also used to pick up other formats:
 *
 * <ul>
 *   <li>
 *       <pre>
 ...
 * </ul>
 *
 * <p>Some validation is done on the extracted text, then the latitude and longitude are extracted
 * and a GeoJSON object is built representing this location.
 *
 * @baleen.javadoc
 */
public class LatLon extends BaleenTextAwareAnnotator {
  ...
  /**
   * Tell the annotator that coordinates are specified with the longitude first rather than the
   * latitude.
   *
   * @baleen.config false
   */
  ...
  /**
   * Use the coordinate translated into decimal as the string to store in the Value field of the
   * Entity, allowing a normalised format for all located coordinates. This option will also format
   * the values to a maximum of 7.d.p. The use of cardinal point strings and the ordering are
  ...
  /**
   * Store the appropriate cardinal points (N, S, E, W) with digital degree values when normalising
   * the text stored in the value field.
   *
   * @baleen.config false
   */
  ...
  /**
   * Store the digital degree values with the longitude first instead of the default latitude first
   * when normalising the text stored in the value field.
   *
   * @baleen.config false
   */
  ...
  /**
   * The minimum number of decimal places required when parsing Decimal Degrees. If 0, then any
   * number of decimal places is accepted.
   *
   * @baleen.config 2
   */
  ...
  /**
   * Set of already found coordinates (per document) to avoid different patterns picking out the
   * same coordinate
   */
  private Set<String> found;

  /**
   * Initialise the annotator - primarily, this sets the regular expression to the correct pattern
   * for the user specified minDP (minimum decimal places)
   */
  @Override
  ...
  /**
   * Extract decimal coordinate pairs from the document, and add validated coordinates to the CAS
   */
  @Override
  public void doProcessTextBlock(TextBlock block) throws AnalysisEngineProcessException {
    found = new HashSet<>();
    String text = normalizeQuotesAndDots(block.getCoveredText());
    ...
    for (Pattern p : patterns) {
      ...
      while (matcher.find()) {
        if (currencySymbols.contains(text.substring(matcher.start(1) - 1, matcher.start(1)))) {
          getMonitor().info("Skipping coordinate as it is preceded by a currency symbol");
          continue;
        }
  ...
  private void processDMS(TextBlock block, String text) throws AnalysisEngineProcessException {
  ...
  private void processDMSText(TextBlock block, String text) throws AnalysisEngineProcessException {
  ...
  private void addCoordinate(
      TextBlock block, Matcher matcher, Double lon, Double lat, String coordinateType) {
    if (lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180) {
      ...
      if (found.add(textLoc)) {
        ...
        loc.setGeoJson("{\"type\":\"Point\",\"coordinates\":" + coords + "}");
