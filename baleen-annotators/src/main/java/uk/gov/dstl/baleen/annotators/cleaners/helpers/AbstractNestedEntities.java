 ...
 *
 * <p>All entities are looped through, and should an entity be found to be entirely contained within
 ...
 *
 * <p>Information may be lost by the removal of entities. Some kind of merging of entities might be
 * a better option.
 *
 * <p>Implementations should override shouldMerge to determine if item should be merged, and the
 * compileEntities to create lists of entities to be considered for merging.
 */
public abstract class AbstractNestedEntities<T extends Entity> extends BaleenAnnotator {
  ...
  private void removeNestedEntities(
      List<T> typeAnnotations, List<MergePair<T>> mergePairs, Set<T> toRemove) {
  ...
  private void removeLeastConfidentEntity(
      List<MergePair<T>> mergePairs, Set<T> toRemove, T e1, T e2) {
  ...
  private void removeEntity(
      List<MergePair<T>> mergePairs, Set<T> toRemove, T entityToRemove, T entityToKeep) {
   ...
   *
   * @param jCas the CAS to pull entities from.
   ...
   *
   * @param keep the entity to be kept
   * @param remove the entity being considered for removal.
   * @return false if both entities should be kept, true is the remove entity should be merged into
   *     the kept.
   */
