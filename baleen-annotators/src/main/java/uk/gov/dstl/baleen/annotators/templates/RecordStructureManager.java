...
public class RecordStructureManager {
  ...
  /**
   * A map to store alteration to the expected structure based on the repeating structural
   * annotations
   */
  ...
  /**
   * Internal recursive method to select the path. This compensates the path for recorded repeating
   * structures.
   *
   * @param node the current node
   * @param selectorParts the (remaining) path to select
   * @return optional of the structure annotation
   */
   ...
   * @throws InvalidParameterException if any of the paths are not valid
   */
  ...
  /**
   * Try to find the given repeating structure after the given preceding structure.
   *
   * @param preceding the preceding structure
   * @param repeatSearch the repeat search
   * @return optional of the found repeating structure
   */
  private Optional<List<Structure>> getRepeat(
      Optional<Structure> preceding, RepeatSearch repeatSearch) {
   ...
   *
   * @param preceding the preceding structure
   * @param repeat the repeat to search for
   * @return optional of the found repeating structure
   */
  private Optional<List<Structure>> searchParts(
      Optional<Structure> preceding, List<SelectorPath> repeat) {
  ...
  /**
   * Check the next structure to see if it matches the expected repeating part.
   *
   ...
   */
  private Optional<List<Structure>> checkNextForMatch(
      SelectorPath path, Optional<Structure> structure) {
   ...
   *
   * @param path the path to match with
   * @param matching the current list of matching structures
   * @param structure the current structure
   * @return optional of the matched repeating structures
   */
  private Optional<List<Structure>> checkNextForMatch(
      SelectorPath path, List<Structure> matching, Optional<Structure> structure) {
    Optional<Structure> nextCheck;
    if (structure.isPresent()) {
      nextCheck = structureHierarchy.getNext(structure.get());
    } else {
      nextCheck =
          structureHierarchy.getRoot().getChildren().stream().map(Node::getItem).findFirst();
    }
  ...
  /**
   * Try to match the given structure path to the selector path given. We first check the path
   * matches the given list then if there are remaining elements of the path we check if the direct
   * children match the path.
   *
   * @param structurePath the starting structure path to match
   ...
   *
   * <p>The returned structure may contain additional empty structures. This aims to make the record
   * extraction more robust to additional whitespace particularly around repeating units.
   *
   * @param preceding the optional preceding point in the structure
   * @param repeatSearch the repeat search
   * @param isFirst set true if this is the first call of a repeating record
   * @return option of the final structure if we can repeat empty if not
   */
  public Optional<Structure> repeatRecord(
      Optional<Structure> preceding, RepeatSearch repeatSearch, boolean isFirst) {
   ...
   *
   * @param preceding the optional preceding point in the structure
   * @param path the repeating path
   * @param end the end of the record
   * @return option of the field structure if we can repeat empty if not
   */
  public Optional<Structure> repeatField(
      Optional<Structure> preceding, SelectorPath path, int end) {
  ...
  /**
   * Record the repeating match of the given structures. These will be used to compensate future
   * calls to {@link #select(String)}.
   *
   * @param structures the repeating structures
   */
