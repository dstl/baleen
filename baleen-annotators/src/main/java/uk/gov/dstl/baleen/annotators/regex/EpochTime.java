...
public class EpochTime extends AbstractRegexAnnotator<Temporal> {
   ...
   *
   * <p>By default, must be after 1st January 2000.
   *
  ...
  /**
   * What's the latest timestamp that is acceptable? A value of -1 will indicate no maximum.
   *
   * @baleen.config -1
   */
  ...
  /**
   * Is the timestamp in milliseconds (rather than seconds). Milliseconds will be converted into
   * seconds (floored), as Baleen does not support timestamps of millisecond resolution.
   *
   * @baleen.config false
   */
  ...
  protected Temporal create(JCas jCas, Matcher matcher) {
    ...
    if (l < earliest) return null;

    if (latest >= 0 && l > latest) return null;
