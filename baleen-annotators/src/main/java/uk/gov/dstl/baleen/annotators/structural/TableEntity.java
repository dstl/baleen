...
/**
 * Extract entities from tables using a regular expression to find columns by their name.
 *
 * <p>The regular expression supplied by the user is run over the table headers (or row 1 if
 * missing) to identify columns. TableCells from that column are then annotated as a user specified
 * type, which must inherit from the Entity class. Users can supply a confidence to assign to
 * annotations created by this annotator.
 *
 * @baleen.javadoc
 */
