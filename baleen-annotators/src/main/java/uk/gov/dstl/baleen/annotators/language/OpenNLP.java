 ...
 *
 * <p>The document content is passed through the OpenNLP Tokenizer, Sentence Detector, Part of
 * Speech Tagger, and Chunker (in that order). The appropriate annotations and properties are added
 * to the CAS, and associations between the relevant annotations (e.g. WordTokens associated with
 * the Sentence) are made.
 *
 * @baleen.javadoc
 */
public class OpenNLP extends BaleenTextAwareAnnotator {
  ...
  public AnalysisEngineAction getAction() {
    return new AnalysisEngineAction(
        ...
        ImmutableSet.of(WordToken.class, PhraseChunk.class, Sentence.class));
  }
   ...
   */
  private List<WordToken> addSentenceWordTokensWithPosTags(Sentence sentIn, TextBlock block)
      throws AnalysisEngineProcessException {
  ...
  private PhraseChunk addPhraseWordsAndHead(PhraseChunk chunk, TextBlock block) {
    ...
    for (int a = constituentWords.size() - 2; a > 1; a--) {
      ...
      if ("IN".equals(wtA.getPartOfSpeech())
