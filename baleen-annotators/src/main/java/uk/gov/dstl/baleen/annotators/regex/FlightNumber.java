 ...
 *
 * <p>Flight numbers are extracted using a regular expression. To minimise false positives, the
 * flight number must be preceded by a variation on the words 'Flight Number'. The regular
 * expression validates the flight number against a list of IATA airline codes (pulled from
 * Wikipedia) followed by a number.
 */
public class FlightNumber extends AbstractRegexAnnotator<Vehicle> {
  private static final String FLIGHTNUMBER_REGEX =
  ...
  public AnalysisEngineAction getAction() {
    return new AnalysisEngineAction(Collections.emptySet(), ImmutableSet.of(Vehicle.class));
  }
}
