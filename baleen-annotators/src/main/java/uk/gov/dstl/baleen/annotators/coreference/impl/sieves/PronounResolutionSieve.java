 ...
 *
 * <p>This is very difficult problem which may perform suboptimally in the current implementation.
 */
public class PronounResolutionSieve extends AbstractCoreferenceSieve {
  ...
  public void sieve() {
    ...
    List<Mention> filteredMentions =
        getMentions().stream().filter(m -> m.getSentenceIndex() >= 0).collect(Collectors.toList());
  ...
  private void resolvePronoun(Mention a, Mention b, Multimap<Mention, Mention> potential) {
    ...
    // Not in paper: We found poor results for "it" really because it never refers to something that
    // Baleen annotates
    ...
    // Originally there was code here to handle a and b both being pronouns (this is described in
    // the paper)
    // However, we are excluding links between two pronouns in validateMentions, so this code was
    // removed.
    ...
    if (sentenceDistance == 0
        && pronoun.getAnnotation().getEnd() <= other.getAnnotation().getBegin()) {
      return;
    }

    if (isPronounPerson(pronoun)
  ...
  private void addBestAsMatch(Mention key, Collection<Mention> potentialMatches) {
    ...
    Optional<Mention> match = matched.stream().filter(m -> !key.overlaps(m)).findFirst();
