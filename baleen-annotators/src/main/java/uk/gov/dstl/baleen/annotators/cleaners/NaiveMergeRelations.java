 ...
 *
 * <p>This is a naive and simple approach which can hide many issues - it is effectively performing
 ...
 *
 * <p>This only really useful if you want to ensure that from a single document you get only a
 * single relationship of the same type, subtype between the same two entities because you want to
...
public class NaiveMergeRelations extends BaleenAnnotator {
  ...
  private boolean isSame(final Relation a, final Relation b) {
    ...
    if (isSame(a.getSource(), b.getSource()) && isSame(a.getTarget(), b.getTarget())) {
      sameSourceTarget = true;
        ...
        && isSame(a.getSource(), b.getTarget())
  ...
  public AnalysisEngineAction getAction() {
    return new AnalysisEngineAction(
        ImmutableSet.of(Relation.class), ImmutableSet.of(Relation.class));
  }
}
