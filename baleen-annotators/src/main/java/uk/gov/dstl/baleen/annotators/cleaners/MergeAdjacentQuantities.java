 ...
 *
 * <p>This annotator will find adjacent quantities of the same type and merge them into a single
 * quantity. For example, 7lb 4oz should be annotated as a single entity, not two.
 */
public class MergeAdjacentQuantities extends MergeAdjacent {
  ...
  public boolean shouldMerge(Entity e1, Entity e2) {
    ...
    return q1.getNormalizedQuantity() >= q2.getNormalizedQuantity()
        && StringUtils.equals(q1.getSubType(), q2.getSubType())
        && StringUtils.equals(q1.getNormalizedUnit(), q2.getNormalizedUnit());
  }

  @Override
  public boolean mergeAdditionalProperties(
      Entity merged, Class<? extends Entity> type, List<Entity> originalEntities) {
    if (type != Quantity.class || merged.getClass() != type) return false;
  ...
  public AnalysisEngineAction getAction() {
    return new AnalysisEngineAction(
        ImmutableSet.of(Quantity.class), ImmutableSet.of(Quantity.class));
  }
}
