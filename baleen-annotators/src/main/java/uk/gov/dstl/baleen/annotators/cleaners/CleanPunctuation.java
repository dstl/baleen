 ...
 *
 * <p>Cleans punctuation by doing the following:
 *
 * <ul>
 *   <li>Removing any characters from the start or end of a string that aren't alphanumeric,
 *       brackets or currency symbols
 *   <li>Counting brackets and if there is a mismatch, and the offending bracket appears at the
 *       start or end of the string, then it is removed
 *   <li>Removing any entities that are empty following the above actions
 * </ul>
 *
 * This annotator handles the value and the covered text of the entity, and will address them
 * separately if required.
 */
public class CleanPunctuation extends BaleenAnnotator {
  private static final String ALLOWED_CHARACTERS_START_END =
  ...
  private void cleanCoveredText(Entity e) {
    ...
    if (openingBrackets > closingBrackets) {
      ...
      for (int i = bothStripped.length() - 1; i > coverText.length() - 1 - diff; i--) {
        if (bothStripped.charAt(i) == ')') {
          end -= 1;
        } else {
          break;
  ...
  private String removeEndPunctuation(String s) {
    ...
    while (ret.length() > 0
        && !ALLOWED_CHARACTERS_START_END.contains(ret.substring(ret.length() - 1))) {
