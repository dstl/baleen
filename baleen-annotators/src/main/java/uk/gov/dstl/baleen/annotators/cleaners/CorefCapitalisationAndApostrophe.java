...
/**
 * Find groups of entities of the same type and same value, ignoring case and apostrophes, and point
 * them at the same ReferenceTarget.
 *
 * <p>If two or more existing ReferenceTargets exist within a group, then the group is either
 * ignored or it's ReferenceTargets merged depending on the mergeReferents parameter. If there is
 * one existing ReferenceTarget within a group then that is used as the ReferenceTarget for all
...
public class CorefCapitalisationAndApostrophe extends BaleenAnnotator {
  /**
   * If two location entities are thought to be coreferences, but they have different existing
   * reference targets, should we merge them?
   *
   * @baleen.config false
   */
  ...
  private ReferenceTarget selectAppropriateReferenceTarget(
      JCas jCas, Collection<ReferenceTarget> referenceTargets) {
