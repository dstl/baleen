...
public class MgrsTest extends AbstractAnnotatorTest{
	...
	public void testWithText() throws Exception{
		...
		jCas.setDocumentText("James has almost certainly never been to 4QFJ1267. But he's been to 4QFJ1268");

		new Text(jCas, 0, 51).addToIndexes();
		// Dont add the second one ... so we should still get 1 results new Text(jCas, 52, jCas.getDocumentText().length()).addToIndexes();
		processJCas();
