...
public class EmbeddedElasticsearch5 {
    ...
    public EmbeddedElasticsearch5(String dataPath, String clusterName) throws NodeValidationException{
        this.dataPath = new File(dataPath);

        Settings settings = Settings.builder()
                .put("path.home", dataPath)
                .put("cluster.name", clusterName)
                //.put("transport.type", "local")       //FIXME: Should this be enabled if we're running a local test node? If we enable it though, the Transport Client doesn't work
