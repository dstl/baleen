 ...
 * own connections to ActiveMQ, instead providing a single instance of
 * MessageProducer and MessageConsumer for Baleen that can be used. This
 * provides benefits such as reduced configuration and reduced repeated code.
 * <br/>
 * Creation of Connection, Session, MessageProducer, and MessageConsumer
 * typically require individual request/response interactions with the broker so
 * are best created upfront.<br/>
...
public class SharedActiveMQResource extends BaleenResource {
	...
	/**
	 * The username to use for authentication. If left blank, then
	...
	/**
	 * The password to use for authentication. If left blank, then
	 * authentication will not be used.
	 *
	 * @baleen.config
	 */
	...
	protected boolean doInitialize(ResourceSpecifier aSpecifier, Map<String, Object> aAdditionalParams) throws ResourceInitializationException {
		try {
			connection = createConnection(activeMQProtocol, activeMQHost, activeMQPortString, activeMQBrokerArgs, activeMQUser, activeMQPass);
			connection.start();
			session = createSession(connection);
			messageProducer = createMessageProducer(session);
		} catch (JMSException jmsException) {
			throw new ResourceInitializationException(new BaleenException("Error connecting to ActiveMQ", jmsException));
		}
	...
	/**
	 * Creates and returns an ActiveMQ session for use in creating
	 * producers/consumers. This method handles the creation of the connection
	 * factory and connection
	 *
	 * @throws JMSException
	 */
	private Connection createConnection(String protocol, String host, String port, String brokerArgs, String username, String password) throws JMSException {
	...
	/**
	 * Create a new message producer, using a null destination so as to allow
	 * multiple destinations on specification at send time
	 * 
	 * @throws JMSException
	 */
	...
	/**
	 * creates and returns a MessageConsumer object consuming from the given
	 * endpoint. Only messages matching the selector are delivered.
	 *
	 * @param queueName
	 *            The name of the queue (or VirtualTopic) to consume from
	 * @param messageSelector
	 *            Only messages matching the selector are delivered
	 */
	public MessageConsumer createConsumer(String queueName, String messageSelector) throws JMSException {
		Destination destination = session.createQueue(queueName);
		return session.createConsumer(destination, messageSelector);
	}

	/**
	 * returns a MessageProducer for use in sending messages. The producer has
	 * no preset destination and can be used to send to any destination
	...
	/**
	 * creates and returns a QueueBrowser object browser the given endpoint.
	 * Only messages matching the selector are delivered.
	 *
	 * @param queueName
	 *            The name of the queue (or VirtualTopic) to consume from
	 ...
	 * @throws JMSException
	 */
	public QueueBrowser createQueueBrowser(String queueName, String messageSelector) throws JMSException {
