 ...
 * Mongo, instead providing a single MongoClient instance for Baleen that can be used. This provides
 * benefits such as reduced configuration, reduced repeated code, and support for connection
 * pooling.
 *
 * @baleen.javadoc
 */
public class SharedMongoResource extends BaleenResource {
  ...
  protected MongoClient createMongoClient(ServerAddress sa, Optional<MongoCredential> credentials) {
    if (!credentials.isPresent()) {
      ...
      getMonitor()
          .debug(
              "Connecting to Mongo with authentication as user '{}'",
   ...
   *
   * @return the monogo db, or null if the source has been destroyed or an initialisation error
   *     occurred.
   */
