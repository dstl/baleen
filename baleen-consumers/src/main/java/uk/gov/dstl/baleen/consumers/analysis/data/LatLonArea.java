...
public class LatLonArea extends LatLon implements Comparable<LatLonArea> {
  ...
  /**
   * Overrides the compareTo operator to compare objects based on area only in order largest to smallest.
   *
   * <p>Note that {@link #equals(Object o) equals} has not been overridden so it is not the case that
   * (x.compareTo(y)==p)==(x.equals(y)) if x and y have the same Area but different locations.
   *
   * @param o the LatLonArea object to be compared.
   * @returns a negative integer, zero, or a positive integer as the the specified object is less than, equal to, or greater than the area of this object.
   */
