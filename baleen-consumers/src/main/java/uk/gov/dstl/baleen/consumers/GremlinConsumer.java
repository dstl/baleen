...
public class GremlinConsumer extends BaleenConsumer {
  ...
  /**
   * The types that should be merged with existing nodes where possible (short name only)
   *
  ...
  protected void doProcess(JCas jCas) throws AnalysisEngineProcessException {
    ...
    for (Metadata md : JCasUtil.select(jCas, Metadata.class)) {
      GraphTraversal<Vertex, Vertex> t =
          buildTraversal(LABEL_METADATA, PROPERTY_KEY, md.getKey(), PROPERTY_VALUE, md.getValue());
    ...
    for (Collection<Entity> entities : targetted.asMap().values()) {
      ...
      for (Entity e : entities) {
        ...
        // See if we have already found the entity to link to with this value and sub-type, in this
        // reference group
          ...
          if (shouldMerge(e)) {
            GraphTraversal<Vertex, Vertex> t =
                buildTraversal(
                    ...
                    e.getValue(),
                    PROPERTY_TYPE,
                    e.getType().getShortName(),
                    PROPERTY_SUBTYPE,
                    e.getSubType());

            if (t.hasNext()) vEntity = t.next();
          }
  ...
  private void addListProperty(Vertex v, String key, List<?> value) {
    if (value != null && !value.isEmpty()) {
      for (Object o : value) v.property(VertexProperty.Cardinality.list, key, o);
    }
  }
