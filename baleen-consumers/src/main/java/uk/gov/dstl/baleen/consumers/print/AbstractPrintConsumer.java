 ...
 *
 * <p>Implementors should use the writeLine functions to output text.
 *
...
public abstract class AbstractPrintConsumer<T extends Base> extends BaleenConsumer {
   ...
   */
  protected <S extends Base> void writeLine(
      StringBuilder sb, FSArray array, Class<S> clazz, Function<S, String> toString) {
  ...
  protected String asString(StringArray array, String separator) {
    return Arrays.stream(UimaTypesUtils.toArray(array)).collect(Collectors.joining(separator));
  }

  /**
   * Override to write the text.
   *
   * <p>Implemetations can use the Write* functions to help.
   *
   * @param t the t
   * @return the string
   */
