...
/**
 * Read messages from an MBOX file, treating messages as plain text and
 * using a content extractor to process attachments.
 *
 * @baleen.javadoc
 */
public class MboxReader extends BaleenCollectionReader {
     ...
     *
     * Note that if it encounters a message bigger than this,
    ...
    @ConfigurationParameter(name = PARAM_IGNORE, defaultValue = {"zip","tar","rar","jpg","gif","png"})
    ...
    protected void doInitialize(UimaContext context) throws ResourceInitializationException {
        ...
        try {
            mboxIterator = MboxIterator.fromFile(mbox).charset(charset).maxMessageSize(messageSize).build().iterator();
     ...
     */
    private void processBinaryBody(JCas jCas, BinaryBody binaryBody, String sourceUrl) throws IOException{
