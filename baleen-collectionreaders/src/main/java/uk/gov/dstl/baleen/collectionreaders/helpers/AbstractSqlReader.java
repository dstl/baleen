...
public abstract class AbstractSqlReader extends BaleenCollectionReader {
  ...
  /**
   * ID column in table - this column will be used to look for new data. It is assumed that this
   * field is numeric and increases as new values are added.
   *
   * @baleen.config id
   */
  ...
  /**
   * The query to list the columns in the database. Column names should be returned in a column
   * called COLUMN_NAME, and any instances of ? will be replaced by the table name.
   *
  ...
  protected Set<String> getColumnNames() {
    ...
    try (ResultSet rsColumns = psListColumns.executeQuery()) {
      ...
      LOGGER.error(
          "Unable to get column names from table {}, empty set will be returned", table, e);
    }
   ...
   *
   * <p>If a startId is provided, then this ID is not included in the dataset (i.e. this should be
   * the last ID you processed). If a startId is not provided (null), then all IDs are returned.
   */
