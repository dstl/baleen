...
/**
 * Creates a logger which will output to a file, either a single file or one which is rotated based on size or time.
 *
 * 
 */
public class BaleenFileLoggerBuilder extends AbstractBaleenLoggerBuilder {
	 ...
	 * @param pattern The logging pattern, even though it isn't set explicitly by this builder it is passed to the super() which might set it
	 ...
	 */
	public BaleenFileLoggerBuilder(String name, String pattern, String file, Filter<ILoggingEvent> filter,
			boolean dailyLogFiles, Optional<Integer> maxSize, Optional<Integer> maxNumberLogs) {
	 ...
	 * @param maxNumberLogs The max number of log files to keep. Defaults to 1
	 */
	public BaleenFileLoggerBuilder(String name, String pattern, String file, List<Filter<ILoggingEvent>> filters,
			boolean dailyLogFiles, Optional<Integer> maxSize, Optional<Integer> maxNumberLogs) {
	...
	private String getMaxFileSize() {
		return maxSize.get() + "KB";
	}

	/**
	 * Build a new appender that will log to file for the specified context and encoder (where required).
	 * 
	...
	protected Appender<ILoggingEvent> createAppender(LoggerContext context, Encoder<ILoggingEvent> encoder) {
		...
		if (!isRolling()) {
			//If the logging isn't configured to roll - i.e. we're not producing daily log files and there is no maximum size set
