...
/**
 * Abstract class implementing the bulk of the RadixTreeGazetteer logic.
 ...
 * @baleen.javadoc
 */
public abstract class AbstractRadixTreeGazetteerAnnotator extends BaleenAnnotator{
	...
	protected InvertedRadixTree<VoidValue> tree = new ConcurrentInvertedRadixTree<VoidValue>(new DefaultCharArrayNodeFactory());
	 ...
	 */
	public AbstractRadixTreeGazetteerAnnotator() {
	}
	...
	public void doInitialize(UimaContext aContext) throws ResourceInitializationException {
		...
		for(String s : gazetteer.getValues()){
			tree.put(s.trim(), VoidValue.SINGLETON);
		}
	...
	public void doProcess(JCas jCas) throws AnalysisEngineProcessException {
		...
		MalleableCharSequence matchText = new MalleableCharSequence(jCas.getDocumentText(), caseSensitive, exactWhitespace);
		Iterable<CharSequence> matches = tree.getKeysContainedIn(matchText);
		
		//deduplicate multiple hits of the same key
		Set<CharSequence> uniqueMatches = new HashSet<CharSequence>();
		
		for (CharSequence match : matches) {
			uniqueMatches.add(match);
		}
		
		for(CharSequence match : uniqueMatches){
			Matcher matcher = matchText.getPatternMatcher(match);
			
			while(matcher.find()){
				try{
					Entity ent = createEntity(jCas, matcher.start(), matcher.end(), matcher.group(), match.toString());
					
					List<String> aliases = new ArrayList<>(Arrays.asList(gazetteer.getAliases(match.toString())));
					aliases.add(match.toString());
					...
					getMonitor().error("Unable to create entity of type {} for value '{}'",entityType.getName(), matcher.group(), e);
					continue;
				}
			}
		}
		...
		if(method.getName().startsWith("set") && method.getName().substring(3, 4).matches("[A-Z]") && method.getParameterCount() == 1){
			...
			if(obj != null && method.getParameterTypes()[0].isAssignableFrom(obj.getClass())){
				try {
					...
					getMonitor().error("Unable to assign property on entity - property will be skipped", e);
				}
			}else if(obj != null && method.getParameterTypes()[0].isAssignableFrom(String.class)){
				getMonitor().debug("Converting gazetteer object of type {} to String", obj.getClass().getName());
				try {
					method.invoke(entity, obj.toString());
				} catch (Exception e) {
					getMonitor().error("Unable to assign property on entity - property will be skipped", e);
				}
			}else if(obj != null && List.class.isAssignableFrom(obj.getClass()) && CommonArrayFS.class.isAssignableFrom(method.getParameterTypes()[0])){
				if(StringArray.class.isAssignableFrom(method.getParameterTypes()[0])){
					try{
						@SuppressWarnings("unchecked")
						List<Object> l = (List<Object>) obj;
						StringArray sa = new StringArray(entity.getCAS().getJCas(), l.size());
