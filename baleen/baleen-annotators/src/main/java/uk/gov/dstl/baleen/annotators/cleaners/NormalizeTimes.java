...
public class NormalizeTimes extends AbstractNormalizeEntities{
	...
	private static final String COULD_NOT_PARSE = "Parse exception occurred at {} for time {}";
	...
	//One of the regex patterns used by the time annotator. Captures 12/24 hour time formats with/without am/pm marker or time zone.
	Pattern p12Hour = Pattern.compile("\\b((([0-1]?[0-9])|2[0-4])[:\\.][0-5][0-9])\\h*((" + TIME_ZONES + ")([ ]?[+-][ ]?((0?[0-9])|(1[0-2])))?)?\\h*(pm|am)?\\b",Pattern.CASE_INSENSITIVE);
	...
	protected String normalize(Entity e) {
		...
		if (mKeyWords.find()) {
			return matchKeywords(mKeyWords);
		} else if (m12Hour.matches()) {
			return match12H(m12Hour, timeString);
		} else if (mSimple.matches()) {
			return matchSimple(timeString);
		} else {
			return matchDefault(timeString);
		}
	}

	private String matchKeywords(Matcher m){
		if (m.group(3) == null) {
			return "12:00";
		} else {
			return "00:00";
	...
	private String match12H(Matcher m, String timeString){
		 ...
		 * ran from 1968 - 1971.
		 */
		StringBuilder timeSb = new StringBuilder("15 " + m.group(1).replaceAll("\\.", ":"));

		try{
			if (m.group(10) != null) {
				timeSb.append(" " + m.group(10));
			...
			getMonitor().warn(COULD_NOT_PARSE, exception.getErrorOffset(), timeSb.toString(), exception);
			return timeString;
		}
		...
		if (m.group(6) != null) {
			String cleanedSubstring = m.group(6).replaceAll("[\\s]", "");
	...
	private String matchSimple(String timeString){
		SimpleDateFormat formatter = new SimpleDateFormat("yy H:m a z");
		Date timeObj = null;
		...
		try {
			...
			getMonitor().warn(COULD_NOT_PARSE, exception.getErrorOffset(), timeString, exception);
			return timeString;
		}
	...
	private String matchDefault(String timeString){
		...
		if ("24".equals(timeString.substring(0,2)))
