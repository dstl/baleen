...
import uk.gov.dstl.baleen.annotators.regex.internals.TimeRegex;
...
public class TimeRegexTest extends AnnotatorTestBase{
	...
	public void beforeTest() throws UIMAException{
		...
		timeRegexAE = AnalysisEngineFactory.createEngine(TimeRegex.class);
	...
	public void timeRegexCountAndValueCheck(Double expectedCount, String... expectedValues) throws Exception{
		...
		assertEquals(expectedCount, Double.valueOf(JCasUtil.select(jCas, Time.class).size()));
		...
		for (int i = 0; i < expectedValues.length; i++) {
			...
			// Get the value
			Time t1 = JCasUtil.selectByIndex(jCas, Time.class, i);
			assertNotNull(t1);
