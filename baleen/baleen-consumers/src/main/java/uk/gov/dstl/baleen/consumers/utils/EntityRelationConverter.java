...
public class EntityRelationConverter {
	 ...
	 */
	public EntityRelationConverter(UimaMonitor monitor, boolean outputHistory, DocumentHistory documentHistory, Set<String> stopFeatures, IEntityConverterFields fields) {
	...
	private void convertFeatures(Map<String, Object> map, Base base) {
		for (Feature f :  base.getType().getFeatures()) {
			...
			try {
				...
				getMonitor().warn(
						"Couldn't output {} to map. Type '{}' isn't supported.", f.getName(), f.getRange().getShortName(), e);
			}
		}
	...
	private void convertFeature(Map<String, Object> map, Base base, Feature f) {
		...
		} else if (f.getRange().isArray() && f.getRange().getComponentType() != null && f.getRange().getComponentType().isPrimitive()) {
	...
	private void saveEvent(List<Object> list, HistoryEvent event, long entityInternalId) {
		...
		if(HistoryEvents.MERGED_TYPE.equalsIgnoreCase(event.getEventType()) && event.getParameters() != null && event.getParameters(HistoryEvents.PARAM_MERGED_ID).isPresent()) {
